...
struct udp_pcb {
    int state;
    struct ip_endpoint local;  // myself
    struct queue_head queue;   // receive queue
    int wc;
};

struct udp_queue_entry {
    struct ip_endpoint foreign;  // dst
    uint16_t len;
    uint8_t data[];
};
...
static int udp_pcb_id(struct udp_pcb *pcb) {
    return indexof(pcbs, pcb);  // index in array
}
...
static void udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface) {
    ...
    mutex_lock(&mutex);
    pcb = udp_pcb_select(dst, hdr->dst);  // search pcb
    if (!pcb) {
        // port is not in use
        mutex_unlock(&mutex);
        return;
    ...
    debugf("queue_pushed: id=%d, num=%d", udp_pcb_id(pcb), pcb->queue.num);
    mutex_unlock(&mutex);
}
...
ssize_t udp_output(struct ip_endpoint *src, struct ip_endpoint *dst, const uint8_t *data, size_t len) {
    ...
    if (len > IP_PAYLOAD_SIZE_MAX - sizeof(*hdr)) {
        // payload capacity overflow check
        errorf("too long");
        return -1;
    }
...
int udp_bind(int id, struct ip_endpoint *local) {
    ...
    mutex_lock(&mutex);
    // correspondence between addr and port
    pcb = udp_pcb_get(id);
    if (!pcb) {
        errorf("udp_pcb_get() failed");
        mutex_unlock(&mutex);
...
ssize_t udp_sendto(int id, uint8_t *data, size_t len, struct ip_endpoint *foreign) {
    ...
    if (local.addr == IP_ADDR_ANY) {
        ...
        if (!iface) {
            errorf("iface not found that can reach foreign addr, addr=%s",
...
ssize_t udp_recvfrom(int id, uint8_t *buf, size_t size, struct ip_endpoint *foreign) {
    ...
    while (1) {
        ...
        pcb->wc++;  // increment wait count
        mutex_unlock(&mutex);
        sleep(1);
        mutex_lock(&mutex);
        pcb->wc--;  // decrement wait count
